...
@router.get("/api/faces/{face_name}/{path}", tags=["faces"])
...
@router.post("/api/faces/rename", tags=["faces"])
async def rename_faces_controller(rename_face_data: RenameFace):
    is_merged = rename_face(rename_face_data.source_face, rename_face_data.dest_face)
    return {"action" : "merge" if is_merged else "rename" }

@router.delete("/api/faces/{face_name}", tags=["faces"])
async def delete_face_controller(face_name):
    delete_face(face_name)

@router.delete("/api/faces/{face_name}/{img}", tags=["faces"])
