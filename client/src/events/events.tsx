...
const Events: React.FC = () => {
  ...
  return (
    <List
      ...
      dataSource={events}
      renderItem={item => (
        <List.Item key={item.id} extra={<Image width={272} alt="logo" src={`${getBaseURL()}/events/${item.id}/img`} />}>
          <Flex vertical>
            <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
              <Flex>
                <Space align="center" style={{ fontSize: '1.1em' }}>
                  <FieldTimeOutlined />
                  <div>
                    {new Date(item.created).toISOString()} - {timeDifference(new Date(item.created), new Date())} ago (
                    {timeDifference(new Date(item.created), new Date(item.done))})
                  </div>
                </Space>
              </Flex>
              <Flex>
                {item.faces.map(face => (
                  <Avatar size={60} src={`${getBaseURL()}/faces/${face.name}/img`} />
                ))}
              </Flex>
              <Flex>
                <Flex vertical>
