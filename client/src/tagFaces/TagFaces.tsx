...
const TagFaces: React.FC = () => {
  ...
  const renameFace = async (faceIndex: number) => {
    if (
      faces[faceIndex].name.toLowerCase() == facesValue[faces[faceIndex].name].toLowerCase() ||
      facesValue[faces[faceIndex].name] == ''
    ) {
      return;
    }
    const response = await axios.post(`${getBaseURL()}/faces/rename`, {
      source_face: faces[faceIndex].name.toLowerCase(),
      dest_face: facesValue[faces[faceIndex].name].toLowerCase()
    });
